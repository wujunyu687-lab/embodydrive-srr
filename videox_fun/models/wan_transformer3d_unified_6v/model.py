...
class UnifiedTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        # Action is used both as AdaLN modulation and as a direct token
        # residual.  The latter prevents a dominant visual-history prior from
        # silently discarding the robot command.
        if "action" in additional_conditions and hasattr(self, "action"):
            action_tokens = self.action.token_residual(additional_conditions["action"])
            tokenized = []
            for batch_index, tokens in enumerate(x):
                frames = int(grid_sizes[batch_index, 0])
                if action_tokens.shape[1] != frames:
                    raise ValueError(
                        f"Action frames {action_tokens.shape[1]} do not match video frames {frames}"
                    )
                if tokens.shape[1] % frames != 0:
                    raise ValueError(
                        f"Token count {tokens.shape[1]} is not divisible by action frames {frames}"
                    )
                tokens_per_frame = tokens.shape[1] // frames
                residual = action_tokens[batch_index].unsqueeze(1).expand(
                    frames, tokens_per_frame, self.dim
                ).reshape(1, tokens.shape[1], self.dim)
                tokenized.append(tokens + residual.to(tokens.dtype))
            x = tokenized
        ...
        with torch.amp.autocast('cuda',dtype=torch.float32):
            ...
            if "action" in additional_conditions and hasattr(self, "action"):
                ...
                e0 = e0 + a0
            if "proprio" in additional_conditions and hasattr(self, "proprio"):
                if len(e0.shape) == 3:
                    e0 = e0.unsqueeze(1).repeat(1, grid_sizes[0][0], 1, 1)
                p0 = self.proprio.modulation(additional_conditions["proprio"])
                e0 = e0 + p0
